...
class Home extends CI_Controller {
	...
	{
		$inventaris = $this->home_model->get_inventaris_where(array('IDinventaris'=>$id));

		$no = explode("/", $inventaris['noInventaris']);
        ...
        $params['data'] = $inventaris['namaBarang']." (".$inventaris['merk'].") [".$inventaris['noInventaris']."]"; //data yang akan di jadikan QR CODE
        $params['level'] = 'H'; //H=High
        $params['size'] = 10;
        $params['savename'] = FCPATH.$config['imagedir'].$image_name; //simpan image QR CODE ke folder assets/img/
        $this->ciqrcode->generate($params); // fungsi untuk generate QR CODE

        echo '<img src="'.base_url('assets/img/qr/').$image_name.'" />';
	}
